...
#define phkoagulan 34
#define pheffluen 35
#define flowpin 13
#define doeffluen 32
#define relayPin_koagulan 33
...
float voltage_effluen, sensorValue_effluen, pH_effluen, phValue_effluen, phRe_effluen, temperature_effluen = 25, phb1_effluen = 7.00, phb2_effluen = 10.01, phv1_effluen = 1770, phv2_effluen = 1118;
float voltage_koagulan, sensorValue_koagulan, pH_koagulan, phValue_koagulan, phRe_koagulan, temperature_koagulan = 25, phb1_koagulan = 7.00, phb2_koagulan = 10.01, phv1_koagulan = 1770, phv2_koagulan = 1118;
float offsite1back = 1, offsite2back = 1, offsite3back = 1, offsite4back = 1, offsite5back = 1, offsite6back = 1, offsite7back = 1;

uint8_t MACmaster[] = { 0x34, 0x86, 0x5d, 0xff, 0x15, 0xcc };  //34:86:5D:FF:15:CC

unsigned int readingId = 0;
int BOARD_ID = 1;
int h_koagulan, f_koagulan, h_effluen, f_effluen, h_defsinfektan, h_bakteri;
...
void setup() {
  ...
  pinMode(relayPin_koagulan, OUTPUT);
  pinMode(relayPin_bad, OUTPUT);
  pinMode(relayPin_good, OUTPUT);
  pinMode(ledindikator, OUTPUT);
  // pinMode(phkoagulan, INPUT);
...
void Task1code(void *pvParameters) {
  ...
  for (;;) {
    ...
    if (currentMillis - previousMillis3 >= 3000) {
      ...
      myepal.ph3 = 0;
      myepal.flow3 = 0;
      myepal.dosing4 = h_koagulan;
      myepal.ph4 = pH_koagulan;
    ...
    if (currentMillis - previousMillis5 >= 60000) {
...
void Get_pH_koagulan() {
  int currentValue_koagulan = 0;
  for (int i = 0; i < 10; i++) {
    currentValue_koagulan += analogRead(phkoagulan);
    delay(10);
  }
  sensorValue_koagulan = (currentValue_koagulan / 10);
  float m_koagulan = (phb2_koagulan - phb1_koagulan) / (phv2_koagulan - phv1_koagulan);
  Serial.println(m_koagulan);
  float c_koagulan = phb2_koagulan - (m_koagulan * phv2_koagulan);
  Serial.println(c_koagulan);
  Serial.print("offsite5 = ");
  Serial.println(offsite5back);
  pH_koagulan = ((m_koagulan * sensorValue_koagulan) + c_koagulan) * offsite5back;
  Serial.print("v koagulan:");
  Serial.println(sensorValue_koagulan);
  Serial.print("ph koagulan:");
  Serial.println(pH_koagulan);
  if (pH_koagulan > 8) {
    digitalWrite(relayPin_koagulan, HIGH);
    Serial.println("lebih dari 8");
    h_koagulan = 1;
  } else {
    digitalWrite(relayPin_koagulan, LOW);
...
void sensorlain() {
  ...
  Liquid_levelko = digitalRead(pinlvkoagulan);
